<?php
	...
	if (isset($_SESSION['login']) && isset($_SESSION['pwd']) ){
		...
		echo "	<nav>
					<ul>
						<li class=\"Menu\"><a>ANNULE SORTIE</a></li>
					</ul>
				</nav>";
		...
		if (isset($_POST['oui']) || isset($_POST['non'])){
			if (isset($_POST['oui'])){
				...
				$resultat = $cnx->query("SELECT dates FROM sortie WHERE nums='".$_GET['nums']."' ");
				foreach($resultat as $valeur){
					$insert = $cnx->exec("DELETE FROM ascension WHERE nums=".$_GET['nums']." AND dateascension='".$valeur['dates']."' ");
				}
